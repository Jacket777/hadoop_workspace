...
public class MyMessageListener implements KafkaMessageListener {
    ...
    public void onMessage(SnKafkaConsumer snKafkaConsumer, MessageRecord messageRecord) {
        ...
        while(iterator.hasNext()){
            System.out.println(" header key: "+iterator.next().key()+"" +
                    " header value: "+iterator.next().value().toString());
            System.out.println("=====下一个header 信息=======");
        }
    }

    @Override
    public void onMessageError(SnKafkaConsumer snKafkaConsumer, MessageRecord messageRecord, Throwable throwable) {
