...
public class Tool {
    ...
    public static void checkResult(boolean isSuccess) {
        if (isSuccess) {
            logger.info("测试通过，创建文件夹成功");
        } else {
            logger.error("测试失败，请检查");
        }
    }
    ...
    public static boolean checkIsPath(String path){
        ...
        if (checkDir(path)) {
            ...
            logger.info("该目录为文件夹目录，且已经存在");
            isExist = true;
        }else if(Tool.checkFile(path)){
            logger.info("该目录为文件目录，且已经存在");
            isExist = true;
        }
        return isExist;
    }
    ...
    public static void checkFileResult(boolean isSuccess) {
        if (isSuccess) {
            logger.info("测试通过，创建文件成功");
        } else {
            logger.info("测试失败，请检查");
        }
    }
    ...
    public static void checkFileIsExist(String path) {
        if (Tool.checkFile(path)) {
            logger.info("该文件已经存在，你将重写该文件");
        } else {
            logger.info("该文件不存在，你将创建新文件");
        }
    }
    ...
    public static void checkCreateFileReplic(String path, short repliaction) {
        if (checkFile(path)) {
            try {
                ...
                logger.info("你创建文件实际副本数： " + number);
                if (repliaction == number) {
                    logger.info("创建文件成功，且副本数正确，用例执行成功");
                } else {
                    logger.info("创建文件成功，但副本数错误，用例执行失败");
                }
            } catch (Exception e) {
                e.printStackTrace();
                logger.info("获取文件信息");
    ...
    public static void checkIsOverWrite(boolean overwrite) {
        if (overwrite) {
            logger.info("你设置的是否重写为:true,如果该文件已存在，则文件将被重写，如果文件不存在则无影响");
        } else {
            logger.info("你设置的是否重写为:false,如果该文件已存在，则将报错，如果文件不存在则无影响");
        }
    }
    ...
    public static long getFileBlockSize(Path filepath) {
        ...
        if (dfs != null) {
            try {
                ...
                e.printStackTrace();
                logger.info("获取文件blockSize失败，请检查");
            } finally {
                return blocksize;
            }
        } else {
    ...
    public static void checkFileIsRight(boolean firstCheck, String path, String content) {
        ...
        if (firstCheck && (result != null)) {
            ...
            if (result.equals(content)) {
                logger.info("重写成功，该用例测试通过！");
            } else {
                logger.info("重写不成功，该用例测试不通过！");
            }
        } else {
            //原先文件不存在，则检查文件是否创建成功，检查文件内容
            if (checkFile) {
                logger.info("该文件新建成功，检查文件内容......");
                if (result.equals(content)) {
                    logger.info("文件内容正确，该用例测试通过！");
                } else {
                    logger.info("文件内容错误，该用例测试不通过！");
                }
            }
        }
...
//                setScope(AclEntryScope.DEFAULT).setType(AclEntryType.MASK).build();
    ...
    public static void getACLInfo(AclStatus aclStatus) {
        if (aclStatus == null) {
            logger.info("AclStatus 对象为空，请检查");
            return;
        }
        logger.info("获取ACL成功");
        ...
        if (aclEntries.size() == 0) {
            logger.info("ACL Entries 为空，请检查");
            return;
        }
        for (int i = 0; i < aclEntries.size(); i++) {
            AclEntry aclEntry = aclEntries.get(i);
            logger.info("aclEntry name is " + aclEntry.getName());
            logger.info("permission name is : " + aclEntry.getPermission().name());
            logger.info("scope name is: " + aclEntry.getScope().name());
            logger.info("type name is: " + aclEntry.getType().name());
            logger.info("============>>>>>================");
        }
    }
